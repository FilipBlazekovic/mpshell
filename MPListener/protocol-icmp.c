...
void startICMPListener()
{
    ...
    // Setting a socket option to receive only ICMP Request packets
    // ------------------------------------------------------------
    ...
    struct iphdr *ipHeaderIN        = (struct iphdr *) packetIN;
    struct icmphdr *icmpHeaderIN    = (struct icmphdr *) (packetIN + sizeof(struct iphdr));
    struct PayloadHeader *headerIN  = (struct PayloadHeader *) (packetIN + sizeof(struct iphdr) + sizeof(struct icmphdr));
    unsigned char *dataIN           = (packetIN + sizeof(struct iphdr) + sizeof(struct icmphdr) + sizeof(struct PayloadHeader));

    /* The response doesn't need the IP header at the
     * begining of the packet, kernel adds the IP header
     */
    struct icmphdr *icmpHeaderOUT   = (struct icmphdr *) packetOUT;
    struct PayloadHeader *headerOUT = (struct PayloadHeader *) (packetOUT + sizeof(struct icmphdr));
    unsigned char *dataOUT          = (packetOUT + sizeof(struct icmphdr) + sizeof(struct PayloadHeader));
    ...
    while (RUN)
    {
        ...
        if (numBytesReceived >= sizeof(struct PayloadHeader))
        {
            ...
            size_t responseSize             = sizeof(struct icmphdr) + PAYLOAD_HEADER_SIZE + ntohs(headerOUT->payloadSize);

            icmpHeaderOUT->type             = ICMP_ECHOREPLY;
            icmpHeaderOUT->code             = 0;
            icmpHeaderOUT->un.echo.id       = icmpHeaderIN->un.echo.id;
            icmpHeaderOUT->un.echo.sequence = icmpHeaderIN->un.echo.sequence;
            icmpHeaderOUT->checksum         = 0;
            icmpHeaderOUT->checksum         = calculateChecksum((unsigned short *) icmpHeaderOUT, responseSize);

            sendto(sockfd, packetOUT, responseSize, 0, (struct sockaddr *)&clientAddress, clientAddressSize);
        }
    }
...
unsigned short calculateChecksum(unsigned short *startAddress, int length)
{
    ...
    unsigned short *currentValue = startAddress;
    ...
    if (bytesRemaining == 1)
    {
        *(unsigned char *) (&oddbyte) = *(unsigned char *) currentValue;
        sum += oddbyte;
    }
