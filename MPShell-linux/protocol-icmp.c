...
int open_icmp_channel(const char *host)
{
    ...
    mp_payload_header *header_in        = (mp_payload_header *) (packet_in + sizeof(struct icmphdr));
    unsigned char *data_in              = (packet_in + sizeof(struct icmphdr) + sizeof(mp_payload_header));
    ...
    uint16_t random_id                  = (uint16_t) (1 + rand()%65535);

    icmp_header_out->type               = ICMP_ECHO;
    icmp_header_out->un.echo.id         = htons(random_id);
    icmp_header_out->un.echo.sequence   = htons(1);
    ...
    while (RUN)
    {
        size_t request_size = sizeof(struct icmphdr) + PAYLOAD_HEADER_SIZE + ntohs(header_out->payload_size);
        sendto(
            sockfd,
            packet_out,
            request_size,
        ...
        process_packet(num_bytes_received, icmp_header_out, header_in, header_out, data_in, data_out);
    }
