...
void openICMPChannel(const char *host)
{
    ...
    struct icmphdr *icmpHeaderIN        = (struct icmphdr *) packetIN;
    struct PayloadHeader *headerIN      = (struct PayloadHeader *) (packetIN + sizeof(struct icmphdr));
    unsigned char *dataIN               = (packetIN + sizeof(struct icmphdr) + sizeof(struct PayloadHeader));
    ...
    uint16_t randomID                   = (uint16_t) (1 + rand()%65535);

    icmpHeaderOUT->type                 = ICMP_ECHO;
    icmpHeaderOUT->un.echo.id           = htons(randomID);
    icmpHeaderOUT->un.echo.sequence     = htons(1);
    ...
    while (RUN)
    {
        ...
        size_t requestSize = sizeof(struct icmphdr) + PAYLOAD_HEADER_SIZE + ntohs(headerOUT->payloadSize);
        sendto(sockfd, packetOUT, requestSize, 0, (const struct sockaddr *) &serverAddress, sizeof(serverAddress));
        ...
        if (numBytesReceived >= sizeof(struct PayloadHeader))
        {
            ...
            if (headerIN->packetID == headerOUT->packetID)
            {
                ...
                uint16_t currentSequenceNum = ntohs(icmpHeaderOUT->un.echo.sequence);
                if (currentSequenceNum >= 65535)
                    currentSequenceNum = 0;
                else 
                    currentSequenceNum++;


                icmpHeaderOUT->un.echo.sequence = htons(currentSequenceNum);
